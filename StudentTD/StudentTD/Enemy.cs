...
namespace StudentTD {
    public class Enemy : GameObject {
        ...
        public Enemy(string imagePath, PointF startPos, float animationSpeed, float scaleFactor, float health, float speed, float dropCurrency, bool flying, bool immune)
        ...
        public override void Update(float deltaTime) {
            ...
            if (position.X + (speed / 100 * 3) >= GameWorld.Checkpoints[cPointChecker].X && position.X - (speed / 100 * 3) <= GameWorld.Checkpoints[cPointChecker].X && position.Y + (speed / 100 * 3) >= GameWorld.Checkpoints[cPointChecker].Y && position.Y - (speed / 100 * 3) <= GameWorld.Checkpoints[cPointChecker].Y) {
                // Enemy is close enough
                // Sets the position to the checkpoint
                position.X = GameWorld.Checkpoints[cPointChecker].X;
                position.Y = GameWorld.Checkpoints[cPointChecker].Y;

                // CHecks if it is the last checkpoint
                if (cPointChecker + 1 < GameWorld.Checkpoints.Count()) {
                    ...
                    // Player loses 1 life
                    GameWorld.LifeCounter -= 1;
                }
            }
            else {
                // Enemy is not close enough
                // Enemy will move towards next checkpoint
                if (position.X != GameWorld.Checkpoints[cPointChecker].X) {
                    if (position.X + 1 > GameWorld.Checkpoints[cPointChecker].X) {
                        position.X -= deltaTime * speed;
                    }
                    else if (position.X - 1 < GameWorld.Checkpoints[cPointChecker].X) {
                        position.X += deltaTime * speed;
                    }
                }
                else if (Position.X + 1 >= GameWorld.Checkpoints[cPointChecker].X && Position.X - 1 <= GameWorld.Checkpoints[cPointChecker].X) {
                    position.X = GameWorld.Checkpoints[cPointChecker].X;
                }
                if (position.Y != GameWorld.Checkpoints[cPointChecker].Y) {
                    if (position.Y + 1 > GameWorld.Checkpoints[cPointChecker].Y) {
                        position.Y -= deltaTime * speed;
                    }
                    else if (Position.Y - 1 < GameWorld.Checkpoints[cPointChecker].Y) {
                        position.Y += deltaTime * speed;
                    }
                }
                else if (Position.Y + 1 >= GameWorld.Checkpoints[cPointChecker].Y && Position.Y - 1 <= GameWorld.Checkpoints[cPointChecker].Y) {
                    position.Y = GameWorld.Checkpoints[cPointChecker].Y;
                }
            }
        ...
        public void OnDeath() {
            if (!dead) {
                ...
                GameWorld.Currency += dropCurrency;
