...
namespace StudentTD {
    public abstract class Tower : GameObject {
        ...
        public Tower(string imagePath, PointF startPos, float animationSpeed, float scaleFactor, float speed, bool placed, bool beingPlaced, bool selected)
        ...
        public override void Update(float deltaTime) {
            ...
            if (beingPlaced) {
                position.X = GameWorld.MousePos.X - Sprite.Width / 2;
                position.Y = GameWorld.MousePos.Y - Sprite.Height / 2;
            }
            ...
            if (placed && target == null) {
                ...
                foreach (GameObject m in GameWorld.Objects) {
                    if ((m as Enemy) != null) {
                        if (((Enemy)m).CurrentHealth <= 0) {
                            ((Enemy)m).OnDeath();
                        }
                ...
                if (target.CurrentHealth <= 0) {
                    ...
                    if (sw.ElapsedMilliseconds > speed || !sw.IsRunning) {
                        if ((this as NormalTower) != null) {
                            GameWorld.NewObjects.Add(new Projectile(@"sprites\towers\normalprojectile.png", new PointF(position.X + (Sprite.Width / 2), position.Y + (Sprite.Width / 2)), 1, 1, (2000 - speed), target.Position));
                            target.CurrentHealth -= damage;
                        }

                        if ((this as AirTower) != null) {
                            if (target.Flying) {
                                GameWorld.NewObjects.Add(new Projectile(@"sprites\towers\airprojectile.png", new PointF(position.X + (Sprite.Width / 2), position.Y + (Sprite.Width / 2)), 1, 1, (2000 - speed), target.Position));
                                target.CurrentHealth -= damage;
                            }
                        }

                        if ((this as SplashTower) != null) {
                            if (!target.Flying) {
                                GameWorld.NewObjects.Add(new Projectile(@"sprites\towers\splashprojectile.png", new PointF(position.X + (Sprite.Width / 2), position.Y + (Sprite.Width / 2)), 1, 1, (2000 - speed), target.Position));
                                foreach (GameObject m in GameWorld.Objects) {
                                    if ((m as Enemy) != null) {
                                        ...
                                        // if a monster is close, it will be set as target
                                        if (mDistance < ((SplashTower)this).SplashRange + 16) {
                                            ((Enemy)m).CurrentHealth -= damage;
                                        }
                                    }
                                }
                            }
                        ...
                        if ((this as SlowTower) != null) {
                            GameWorld.NewObjects.Add(new Projectile(@"sprites\towers\slowprojectile.png", new PointF(position.X + (Sprite.Width / 2), position.Y + (Sprite.Width / 2)), 1, 1, (2000 - speed), target.Position));

                            //If the targeted enemy is not slowed and is not immune the enemies is slowed
                            if (!target.IsSlowed) {
                                if (!target.Immune) {
                                    target.Speed = target.Speed * ((SlowTower)this).SlowSpeed;
                                    target.SlowMonster();
                                }
                            }
                            ...
                            //If that is less than the acutal target speed a new slowSpeed is given. 
                                ...
                                if (target.TemporarySpeed * ((SlowTower)this).SlowSpeed < target.Speed) {
            ...
            if (selected || beingPlaced) {
                ...
                if (selected) {
                    ...
                    if ((this as SplashTower) != null) {
                        ...
                        f = new Font("Arial", 8, FontStyle.Regular);
                        dc.DrawString(((SplashTower)this).SplashRange.ToString(), f, Brushes.Black, 640, 370);
                    }

                    // Shows the current slowrange for a selected slowtower
                    else if ((this as SlowTower) != null) {
                        f = new Font("Arial", 8, FontStyle.Bold);
                        dc.DrawString("Slow: ", f, Brushes.Black, 561, 370);
                        f = new Font("Arial", 8, FontStyle.Regular);
                        dc.DrawString((Math.Round((1 - (((SlowTower)this).SlowSpeed)) * 100, 0)).ToString() + "%", f, Brushes.Black, 595, 370);
                    }
                    ...
                    if ((this as SlowTower) == null && upgradeLevel < 20 || (this as SlowTower) != null && upgradeLevel < 5) {
                        ...
                        if ((this as SplashTower) != null) {
                            ...
                            f = new Font("Arial", 8, FontStyle.Regular);
                            dc.DrawString((((SplashTower)this).SplashRange + 5).ToString(), f, Brushes.Black, 760, 371);
                        }
                        // Shows the new slowspeed  for enemies in % if a selected splashtower is upgraded.
                            ...
                            f = new Font("Arial", 8, FontStyle.Regular);
                            dc.DrawString((Math.Round((1 - (((SlowTower)this).SlowSpeed)) * 100, 0) + 5).ToString() + "%", f, Brushes.Black, 715, 371);
                        }
        ...
        public void OnSelection() {
            ...
            if ((this as SlowTower) == null && upgradeLevel < 20 || (this as SlowTower) != null && upgradeLevel < 5) {
                GameWorld.NewObjects.Add(new Upgrade(@"sprites\buttons\upgrade.png", new PointF(650, 440), 0, 1, 1, this));
            }

            GameWorld.NewObjects.Add(new Sell(@"sprites\buttons\sell.png", new PointF(570, 440), 0, 1, 1, this));
        }
